    ...
    public sealed class CloudAtlasGeneratorWindow : EditorWindow
    {
        ...
        static class Styles
        {
            ...
            public static readonly GUIContent PreviewCoverage = new GUIContent("Coverage",
                "Preview only — it is not baked. Mirrors Cloud Coverage on the volume with the same " +
                "threshold the sky uses, so what you see here at a given value is what the sky " +
                "shows at that value. At 1 the sky is fully overcast.");
        ...
        void ReadBrush()
        {
            ...
            m_BrushPixels = readable.GetPixels();
        ...
        void DrawShape()
        {
            ...
            m_Brush = (Texture2D)EditorGUILayout.ObjectField(
                new GUIContent("Brush Atlas",
                    "A brush atlas made by the Brush Atlas Generator. Its strokes are baked into " +
                    "the cloud: Brush cuts their tone into the density, Paint rebuilds the cloud out " +
                    "of them. Any other texture will not work for Paint, since Paint reads the " +
                    "stroke field only a brush atlas carries."),
            ...
            using (new EditorGUI.IndentLevelScope())
            {
                ...
                EditorGUILayout.PropertyField(Find(nameof(CloudAtlasSettings.paint)),
                    new GUIContent("Paint"));
