...
namespace HiddenBull.UrpStyle.Editor
{
    public sealed class CloudAtlasGeneratorWindow : EditorWindow
    {
        ...
        {
            var signature = $"{JsonUtility.ToJson(m_Settings)}|{m_PreviewCoverage}|{m_PreviewSoftness}";

            if (m_Preview != null && signature == m_PreviewSignature)
                return;

            var atlas = CloudAtlasBaker.Bake(m_Settings, k_PreviewResolution);
            var shaded = CloudAtlasBaker.ShadePreview(
                atlas, k_PreviewResolution, m_PreviewCoverage, m_PreviewSoftness, out m_Coverage);
